...
export function ChatView() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full">
      ...
      {/* ── "In memory" chip — sticky above messages during active processing ── */}
      {hasActivity && (
        <div className="px-3 md:px-4 pt-2 pb-1 flex flex-col gap-1.5 border-b border-purple-500/10 bg-gradient-to-b from-purple-900/8 to-transparent">
          <button
            onClick={() => setTraceOpen(v => !v)}
            className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-purple-600/12 via-blue-600/8 to-honey-500/8 border border-purple-500/25 w-fit max-w-full hover:border-purple-500/40 transition-all"
          >
            <Brain size={12} className="text-purple-400 shrink-0 animate-pulse" />
            <span className="text-[11px] text-purple-300/90 font-medium truncate max-w-[300px]">
              {longTaskLabel
          ...
          {traceOpen && streamTraces.length > 0 && (
            <div className="ml-2 flex flex-col gap-1 border-l-2 border-purple-500/20 pl-3 pb-1">
